...
static upa::url_result ipv4_parse(const char* szInput, uint32_t& ipv4) {
...
TEST_CASE("IPv4 parser test with empty input") {
    ...
    uint32_t ipv4 = 1;
    CHECK(upa::ipv4_parse_number(szEmpty, szEmpty, ipv4) != upa::url_result::Ok);

    // https://url.spec.whatwg.org/#concept-ipv4-parser
    // 6. 1. Let result be the result of parsing part.
    // 6. 2. If result is failure, validation error, return failure.
    CHECK(upa::ipv4_parse(szEmpty, szEmpty, ipv4) != upa::url_result::Ok);
}
...
TEST_CASE("IPv4 parser test with 127.0.0.1") {
    ...
    ipv4 = 0;
    CHECK(ipv4_parse("0x7f000001", ipv4) == upa::url_result::Ok);
    CHECK(ipv4 == 0x7f000001);

    ipv4 = 0;
    CHECK(ipv4_parse("0x7f.0.0.1", ipv4) == upa::url_result::Ok);
    CHECK(ipv4 == 0x7f000001);

    ipv4 = 0;
    CHECK(ipv4_parse("127.0.0.1", ipv4) == upa::url_result::Ok);
    CHECK(ipv4 == 0x7f000001);

    ipv4 = 0;
    CHECK(ipv4_parse("127.0.1", ipv4) == upa::url_result::Ok);
    CHECK(ipv4 == 0x7f000001);

    ipv4 = 0;
    CHECK(ipv4_parse("127.1", ipv4) == upa::url_result::Ok);
    CHECK(ipv4 == 0x7f000001);
}
